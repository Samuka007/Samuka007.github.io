...
（[TacMamba] 则是完全 SSM 的实现了，transformer 是因为本身实现了很强的 performance 所以才让 SSM 很难从 0 scale 到一个比肩 transformer 的准确性，那 VLA 这个众生平等的起点 SSM 有没有可能反超呢？）
...
[DexWorldModel] 用 DINOv3 做 static feature extractor，再由 video - action model 来推理，同时维护一个 test-time-training 层在每一帧新到达时进行更新。感觉之后高性能任务用 DINOv3 会不会不太够用？输出的特征应该会主动去忽略阴影、噪声等动态。（比如，打 CS 时可能由于特征图读取的问题，有些时候明明先漏阴影可以预判击杀，但却因为 static feature 屏蔽掉了，没能意识到这一点）

不过初步实现可能可以给 Video ViT 省下较多预算？
